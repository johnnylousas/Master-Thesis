...
class DataCI(Data, Visualizer):
    ...
    def __init__(self, commits, test_details, test_history, mod_files, start_date: str = '2019-03-11 00:00:00.000000',
        ...
        # remove last df rows for predicting on unseen data
        self.df_unseen = self.df_link.tail(self.predict_len)
        self.df_link.drop(self.df_link.tail(self.predict_len).index,
                          inplace=True)

        # create pairs
        self.pairs = self.create_pairs(files=False)
    ...
    def create_pairs(self, files: bool = False):
        pairs = []
        if files:
            ...
            self.df_link = self.df_link.explode('name')
            #self.df_link.mod_files = pd.DataFrame(self.df_link.mod_files.values.tolist()).agg(list, 1)
            for row in self.df_link.iterrows():
                files = []
                for file in row[1]['mod_files']:
                    if file is not None:
                        files.append(self.file_index[file])
                    else:
                        files.append(0)
                row[1]['name'] = self.test_index[row[1]['name']]
                pairs.append((tuple(files), row[1]['name']))
        print(pairs[:5])
        #print(sorted(set(pairs), key=pairs.index)[:5])
        print(f'there are {len(pairs)} pairs and {len(set(pairs))} are unique')
        return pairs
