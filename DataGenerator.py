...
class DataGenerator(keras.utils.Sequence):
    ...
    def data_generation(self, pairs):
        ...
        while True:
            ...
            batch_revisions = tf.keras.preprocessing.sequence.pad_sequences(batch[:, 0], padding='post').astype('float32')
            batch_tests = np.asarray(batch[:, 1], dtype='float32')
            batch_labels = np.asarray(batch[:, 2], dtype='float32')
            yield {'file': batch_revisions, 'test': batch_tests}, batch_labels
