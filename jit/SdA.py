...
def test_SdA(pretrain_lr=0.1, pretraining_epochs=1000, corruption_level=0.3, \
    ...
    filename = 'bugzilla'
    data: pandas.DataFrame = pandas.read_csv('input/' + filename + '.csv')
    date = data.pop('transactionid').values
    date = data.pop('commitdate').values
    Y: numpy.ndarray = data.pop('bug').values
    ...
    # construct SdA
    sda = SdA(input=X_train, label=Y_train, \
              n_ins=14, hidden_layer_sizes=[12, 12], n_outs=1, numpy_rng=rng)
